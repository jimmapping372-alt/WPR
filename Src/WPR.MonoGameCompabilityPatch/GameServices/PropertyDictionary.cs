...
namespace WPR.MonoGameCompability.GamerServices
{
      ...
      IEnumerable
    {
        ...
        private T GetTypedValue<T>(string key) where T : struct, IEquatable<T>
        {
            ...
            return property is TypedPropertyValue<T> typedPropertyValue 
                ? typedPropertyValue.GetTypedValue() 
                : (T)Convert.ChangeType(property.GetValue(), typeof(T), (IFormatProvider)CultureInfo.InvariantCulture);
        }
